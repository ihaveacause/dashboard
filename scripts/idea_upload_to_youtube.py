...
def get_episode(episode_number: int, language: str) -> dict:
    ...
    table = "tamil_ideas" if language == "tamil" else "english_ideas"
...
def calculate_publish_time(language: str) -> datetime:
    ...
    """
    # Ideas are one-offs, not a daily series drip — schedule each to go public
    # ~1 hour after upload. Prompt, predictable, and isolated from the series
    # schedule. (Adjust the hours, or just publish manually in YouTube Studio.)
