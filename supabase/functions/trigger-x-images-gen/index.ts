...
serve(async (req) => {
  ...
  try {
    const { episode_number, idea_id } = await req.json();
    ...
    const response = await fetch(
      `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/actions/workflows/generate_x_images.yml/dispatches`,
      {
        method: "POST",
        headers: {
    ...
    if (response.status === 204) {
      return new Response(
        JSON.stringify({ success: true, message: "X image generation triggered — 6 images per language" }),
