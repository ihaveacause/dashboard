...
serve(async (req) => {
  ...
  try {
    const { episode_number, idea_id, language = "ta", youtube_link = "" } = await req.json();
    ...
    if (youtube_link)   inputs.youtube_link = youtube_link;

    const response = await fetch(
      `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/actions/workflows/post_to_x.yml/dispatches`,
      {
        method: "POST",
        headers: {
    ...
    if (response.status === 204) {
      return new Response(
        JSON.stringify({ success: true, message: "X posting triggered" }),
