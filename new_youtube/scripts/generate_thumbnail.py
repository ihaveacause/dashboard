...
def main():
    ...
    # Find the episode's sharpest idea, then THUMB_CANDIDATES radically-different hook+picture pairs.
    ...
    try:
        ...
        for i, pair in enumerate(pairs, start=1):
            ...
            try:
                ...
                gcs_path = f"episodes/{EPISODE_NUMBER:03d}/{lang_code}/cand_{i:02d}.jpg"
        ...
        log(f"\n{'='*60}")
        log(f"✅ Episode {EPISODE_NUMBER} {LANGUAGE.upper()} — {len(candidates)} thumbnail candidates ready")
        log(f"{'='*60}")
