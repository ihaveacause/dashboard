...
# Burned-in karaoke captions OFF by default (text-free master). Set BURN_CAPTIONS=1 to restore.
...
def synthesize_chirp3(script_text, voice_name, out_path, tmpdir):
    ...
    Chirp 3 HD takes plain text only (no SSML / pitch / rate). Long scripts
    are chunked and concatenated."""
    import base64
...
def main():
    ...
    log("="*60)
    log(f"🎬 Video Generator (CTC) — Episode {IDEA_NUMBER} | {LANGUAGE.upper()}")
    log(f"   {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log("="*60)

    table   = "tamil_ideas" if LANGUAGE == "ta" else "english_ideas"
    ep_rows = db_get(table, {"episode_number": f"eq.{IDEA_NUMBER}", "select": "*"})
    episode = ep_rows[0] if ep_rows else None
    if not episode:
        log(f"❌ Episode {IDEA_NUMBER} not found in {table}"); return
    log(f"   ✅ {episode.get('title_english') or episode.get('title_tamil')}")
    db_patch(table, IDEA_NUMBER, {"status": "generating_video"})
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if tts_voice:
            ...
            if not script_text.strip():
                log("❌ No script for TTS"); db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
            if not synthesize_chirp3(script_text, tts_voice, voice_path, tmpdir):
                db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
        else:
        ...
        if BURN_CAPTIONS:
            ...
            if build_srt(words, srt_path):
                ...
                cap_url = upload_to_gcs(srt_path, f"ideas/idea{IDEA_NUMBER:03d}/{cap_lang}/captions.srt",
                                        content_type="application/x-subrip")
                if cap_url:
                    db_patch(table, IDEA_NUMBER, {"captions_url": cap_url})
        ...
        if not raw:
            log("❌ No images"); db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
        ...
        if not image_paths:
            log("❌ No images downloaded"); db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
        ...
        output_path = os.path.join(tmpdir, f"ep{IDEA_NUMBER:03d}_{lang_code}.mp4")
        if not render_video(frames_concat, voice_path, music_path, intro_path, outro_path,
            ...
            log("❌ Render failed"); db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
        log(f"   ✅ Render done — {datetime.now().strftime('%H:%M:%S')}")

        log(f"\n☁️  Uploading...")
        video_url = upload_to_gcs(output_path, f"ideas/ep{IDEA_NUMBER:03d}/{lang_code}/final.mp4")
        if not video_url:
            log("❌ Upload failed"); db_patch(table, IDEA_NUMBER, {"status": "voice_approved"}); return
        db_patch(table, IDEA_NUMBER, {"video_url": video_url, "status": "video_ready"})
        log(f"\n{'='*60}")
        log(f"✅ Episode {IDEA_NUMBER} {LANGUAGE.upper()} — video ready!")
